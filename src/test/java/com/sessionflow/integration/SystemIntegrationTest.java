...
class SystemIntegrationTest {
        ...
        void testDataValidationWorkflow() {
                ...
                // 嘗試創建同名標籤應該失敗
                assertThatThrownBy(() -> tagService.createTag(new TagRequest("唯一標籤", "#000000")))
                                .isInstanceOf(Exception.class);

                // 測試無效的顏色格式
                assertThatThrownBy(() -> tagService.createTag(new TagRequest("無效顏色標籤", "invalid-color")))
                                .isInstanceOf(Exception.class);

                // 測試任務標題不能為空
                assertThatThrownBy(() -> taskService.createTask(new TaskRequest("")))
                                .isInstanceOf(Exception.class);
                ...
                assertThatThrownBy(() -> scheduleEntryService.createScheduleEntry(invalidSchedule))
                                .isInstanceOf(Exception.class);
        }
